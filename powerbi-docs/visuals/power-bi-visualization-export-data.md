...
- Report authors can apply [sensitivity labels](../admin/service-security-data-protection-overview.md) to reports. These labels applied on files when data is exported to Excel, PowerPoint, and PDF files. If sensitivity label has protection settings these settings will be applied on files when export data. Only authorized users will be able to open files with protection.
